...
<form method="post" action="{{ route('salary.store') }}" class="row row-cols-lg-auto g-3 align-items-center">
    ...
    <div class="form-group col-sm-6 col-md-2 row">
        <input type="number" class="form-control col-4 form-control-sm" name="ngay"
            placeholder="{{ __('default.day') }}" />
        <input type="number" class="form-control col-4 form-control-sm" name="thang"
            placeholder="{{ __('default.month') }}" value="{{$user->thang}}" />
        <input type="number" class="form-control col-4 form-control-sm" name="nam"
            placeholder="{{ __('default.year') }}" value="{{$user->nam}}" />
    </div>
